async function productCompInit() {
    ...
    for (let i = 1; ; i++) {
      ...
      if (p[key]) imgs.push(p[key]); else break;
    }
...
lb.addEventListener('touchend', e => {
  ...
  const now = Date.now();
  if (now - lastTap < 300) { scale = 1; ox = 0; oy = 0; applyTransform(); }
  lastTap = now;
}, {passive:true});
    ...
    if (!suggestions.length) {
      ...
      viewMore.href = `search.html?q=${encodeURIComponent(p.name)}`;
      viewMore.style.cssText = 'display:block;margin:1rem 0 0;padding:.8rem;text-align:center;background:#fff3ee;color:#FF5400;font-weight:700;font-size:.95rem;border-radius:.6rem;text-decoration:none;border:1.5px solid #FF5400;';
      viewMore.textContent = 'View more of this kind →';
      document.getElementById('pc-suggestions').appendChild(viewMore);
    }
}